...
export function TopBar({ inApp = false }: { inApp?: boolean }) {
  return (
    <header className="sticky top-0 z-30 border-b border-border bg-background/85 backdrop-blur-md">
      <div
        className={`${inApp ? "px-3 sm:px-4 md:px-6" : "max-w-6xl mx-auto px-4 md:px-6"} h-14 flex min-w-0 items-center justify-between gap-3`}
      >
        <Link to="/" className="flex min-w-0 items-center gap-2">
          <Mark />
          <span className="truncate font-serif text-lg leading-none">CipherLend</span>
